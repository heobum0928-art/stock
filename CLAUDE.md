...
**숫자를 인용할 때 반드시 잣대를 같이 적는다** — "+4.80%"가 아니라 "+4.80%(명목가 기준)".
2026-08-23에 이 규칙을 적어놓고 바로 다음 과제에서 또 틀렸다(명목 +4.80%를 "증거금 기준"
이라고 에이전트에 넘김 → 규명해야 할 격차가 6.8%p가 아니라 14.4%p였음).

### `data/margin_short_trades.csv`의 알려진 결함 (2026-08-23 발견, 미수정)

판정 입력값을 오염시킨다. 이 파일을 쓰기 전에 반드시 확인할 것:
1. **`pnl_pct`에 펀딩비·수수료가 없다** — 건당 -3.31%p 누락. 순손익은 원장을 봐야 한다.
2. **헤더는 16필드인데 데이터는 18필드** — `listing_age_days`, `qvol_24h`가 무명 컬럼.
   pandas 기본 파싱이 깨진다.
3. **`pnl_pct`가 현물·선물 혼합** — 진입가는 선물 실체결가인데 청산가는 현물 티커다.
   스탑·트레일링 판정도 현물가로 한다. 원장과 건당 최대 88%p 어긋난다.
4. 원장 오염(12h 내 재거래 매칭 오류)이 CSV에도 존재 — TST×2, TUT×2.
