...
ALLOW_PATTERNS = ["캐스케이드 진입", "캐스케이드 청산", "캐스케이드 매수 실패", "캐스케이드 매도 실패", "🚨", "[포지션현황]", "[추세조기경보]", "[크립토브리핑]", "증거금 -", "판 뒤", "[예측대조]", "급등 쏠림", "손절 주의", "손절 위험", "손절 손절임박", "숏 쏠림", "[CUSUM롱후보]", "[CUSUM롱결과]", "[시장폭]"]
...
def send(text: str, force: bool = False) -> bool:
    ...
    if "🚨" in text or "증거금 -" in text or "손절 주의" in text:
        # ★ 2026-09-22(사용자 요청으로 손실 원인 재분석 중 발견): 손절 조기경보 1단계
        # "⚠️ 손절 주의"(-20%명목)만 🚨가 없어 무음시간(00~06)에 조용히 사라지고 있었다.
        # 2·3단계(위험/손절임박)는 🚨라 이미 새벽에도 갔다. 1단계도 같은 경로로 보낸다.
        force = True
