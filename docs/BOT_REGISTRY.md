...
| `quietpump_long_paper.py` | **롱** | ★2026-08-26 신설. 조용한급등 — 봉인검정 7기준 전부 통과한 첫 후보(단, 24h확정으로 그 검정 효력 상속 안 함, 위 참조) | 20쌍(목표 60쌍) | **9/19 소량파일럿 체크포인트 / 10/26 완전판정** |
| `prom_long_paper.py` | **롱** | ★2026-08-30 신설. PROM 실거래 숏 6건 3승3패 -66.58 확인 후, "숏이 반복해서 크게 깨지면 롱이 낫나"를 PROM 단일종목으로만 검정. 사전 예측 3:7 불리 — 미러롱(n=1228)이 이미 두 번 기각된 것과 같은 논리 구조라서 | 0건(신설, 신호 드묾 — 3일에 1건꼴) | **11/15** |
