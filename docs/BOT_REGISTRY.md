...
> 그때 같이 맞춰야 한다.**
>
>> ★★ **2026-09-04 정정 — 아래 "미구현" 주장은 틀렸다. `global_cap_usdt`는 작동한다.**
>> `binance_guard.py:313`이 `margin > global_cap_usdt`면 차단한다. 다만 **엔진 합산이 아니라
>> 주문 1건의 증거금**과 비교한다 — 그래서 건당 80~250인 숏에는 한 번도 안 걸렸고("무해"는
>> 그 이유였다), 건당 증거금이 큰 `core_lev`에는 걸린다. 2026-09-04 실측: core_lev 캡을
>> 500으로 올리자 목표명목 1,000(증거금 500)이 `전체 상한 초과(500.0>280)`로 차단됐다.
>> **엔진캡을 280 넘게 올릴 때는 `global_cap_usdt`도 같이 올려야 그 캡이 실제로 쓰인다.**
>> 아래 "280 → 950으로 맞춘다"는 실제로 반영되지 않았다(현재 값 280).
>
> ★ **2026-09-02 갱신 — 실질 상한은 `global_cap_usdt`가 아니라 엔진캡 합계다.**
> 건당 증거금 증액(원본 50→**250**, 완화 50→**80**)과 캡 조정(`mshort_fut` 350→**500**)으로
> armed 엔진 합계가 **950 USDT**가 됐다(core_lev 100 + mshort_fut 500 + mshort_wide_fut 350).
> `global_cap_usdt`는 미구현이라 값이 무엇이든 동작이 바뀌지 않으므로, **혼동 방지를 위해
> 280 → 950으로 맞춘다**(안전장치를 푸는 게 아니라 문서를 현실에 맞추는 것).
>
> **왜 global_cap 버그를 고치지 않기로 했나(2026-09-02 결정)**: 안전장치가 이미 두 겹이다 —
> (1) 엔진별 캡이 실제로 작동하고, (2) 엔진별 일일손실한도(200)가 폭주를 끊는다. 반면
> global_cap을 280 그대로 살리면 원본봇 1건(250)만으로 거의 소진돼 완화봇이 마비된다.
> 새 숫자를 임의로 정하는 것도 근거 없는 파라미터를 늘리는 일이라 하지 않는다.
>
> **현재 최대 노출**: 증거금 950(원본 250×2 + 완화 80×4 + BTC롱 ~116 ≈ 936).
> 전부 손절 시 약 -760 USDT = 선물지갑(~1,390)의 55%. 증액 전에는 -640(46%)였다.
...
|---|---|---|---|---|
| `shadow_fleet.py` | 숏 | 청산규칙 짝비교 V0~V7 (★09-04: V3·V4 퇴역, V6 시간손절·V7 펀딩청산 신설 — `PREREG_FLEET_V6V7.md`) | 359행 / 신호 89개 (09-04) | 9/19 |
| `status_heartbeat.py` | — | 30분마다 텔레그램 상태 심박(포지션+봇생존+지갑). 읽기전용, ONESHOT. 봇 중단 시 🚨로 무음시간 강제발송 | ★2026-09-06 신설 | — |
| `forecast_scorecard.py` | — | 주간 예측 vs 실제 대조표(`data/forecast_baselines.json` 고정 예측 대비 원장 실적). 읽기전용, ONESHOT 7일 | ★2026-09-05 신설 | — |
| `bc_rule_shadow_paper.py` | 숏 | 실전 진입 미러링 + 손실축소 규칙 | 20건(목표 20 도달) | 9/2 **★09-04 중단(사용자 결정)** |
| `oi_divergence_short_paper.py` | 숏 | OI 다이버전스 필터 | 12건 | 9/2 **★09-04 중단(사용자 결정)** |
| `alt_momentum_long_paper.py` | **롱** | 알트 모멘텀 Top3 (100건 목표) | 20건 | 10월 중순 **★09-04 중단(사용자 결정)** |
