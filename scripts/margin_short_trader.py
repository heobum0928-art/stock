...
STOP_PCT = 5.0            # ★ 2026-09-22 사용자 결정("10%로 하자", 증거금 -10%): PREREG_STOP10.md —
                          ...
                          # 이전값 40.0(증거금 -80%). 진입가 대비 +5% 상승 시 손절(2배 청산선 +50% 안쪽)
...
# ★ 2026-09-01: 사용자 지시로 50→100(2배). 배경: 필터 풀세트(V1+CUSUM+횡보장+펀딩청산)
#   적용 후 신호가 이틀 1건꼴로 줄어 동시 포지션이 보통 1~2개 — 캡이 아니라 건당 크기가
#   실투입을 결정한다. 손절 1건 손실 -40→-80 USDT(증거금 -80% 기준). 사용자가 총 500
#   배정 의사로 결정, 엔진캡도 350→500으로 함께 올림(사용자 직접 수정). 일일손실한도
#   200은 유지 — 하루 2번 손절이면 그날 정지되는 구조(의도된 안전선).
FUT_MARGIN_PER_TRADE = 120.0
# ★ 2026-09-17 사용자 지시("신규진입 중단하자"): 신규진입만 차단. 기존 포지션의 손절·만기·
...
# ★★ 2026-09-02 철회 — 검증 재현 결과 이득의 전부가 단일 사건이었다.
#   재현: 짝차이 +1.180%p(기록 +1.256), CI[+0.522,+1.914], 발동 933건 — 수치는 맞다.
#   그런데 날짜별로 쪼개면:
#     2025-10-11 하루(285건)  → +10.15%p = **총이득의 102%**
#     그날 제외(2,114건)      → **-0.029%p**, CI[-0.83,+0.77] (훈련 -0.591 / 홀드 +1.642 부호불일치)
#     상위 5일 제거           → -0.300%p
#   월별로도 12개월 중 7개월만 양수(무작위 수준).
#   ★ 이 저장소는 같은 날(09-01) 오토리서치에서 **시간손절**을 "효과의 76%가 10-10 대폭락
#     하루에 집중"이라는 이유로 REFUTED 기각했다. 펀딩청산은 102%로 더 심한데 채택했다 —
#     같은 잣대를 서로 다르게 적용한 것이므로(CLAUDE.md 5항) 철회한다.
#   되살리려면 단일사건 의존을 통과할 새 근거가 필요하다(문턱 조정은 격자스캔에 해당).
FUND_EXIT_ENABLED = False
...
# ★ 2026-09-17 사용자 결정으로 OFF. 적용 후(08-31~) 원본봇 진입이 17일간 3건으로 급감,
#   AIN(-206)이 결과를 지배. 12/10 CUSUM 정식판정(PREREG_CUSUM_JUDGE)은 그림자함대 데이터라 영향 없음.
CUSUM_ENABLED = False
    ...
    while True:
        try:
            ...
            for sym in list(positions.keys()):
                ...
                if pos.get("live") and now - pos.get("_ext_chk", 0) >= EXT_CHECK_SEC:
                    ...
                    if exchange_position_gone(sym, pos["coin"], pos.get("venue", "margin"),
                        ...
                        continue
                    # ★ 2026-09-22(사용자 요청 "봇으로 중간중간 체크", 완화봇과 동일 로직): 포지션은
                    #   살아있는데 실제 수량·평단이 봇 기록과 다른 경우(수동 불타기) 장부 동기화.
                    #   안전 자체는 이미 확보돼 있다 — place_protective_stop()이 closePosition=true라
                    #   수량과 무관하게 트리거 시 전체 청산된다. 이 블록은 봇의 손익 계산이 낡은
                    #   수량·평단을 쓰지 않도록 **기록만** 맞춘다. 손절 재등록은 하지 않는다.
                        ...
                        if rp and abs(rp["amt"]) > 0:
                            ...
                            if pos.get("qty") and abs(real_qty - pos["qty"]) > pos["qty"] * 0.01:
                                ...
                                try:
                                    notify.send(f"🔧 {sym} 포지션 수량 변경 감지(수동 조작 추정) — "
            ...
            for coin in (UNIVERSE_SET | FUTURES_UNIVERSE):
                ...
                # ★ 2026-09-17: 신규진입 차단 스위치 — 신규상장 모의롱 기록(위)은 유지하고
                #   실제 돈이 나가는 진입만 막는다(근거는 NEW_ENTRY_ENABLED 정의부 주석).
                if not NEW_ENTRY_ENABLED:
                    log.info(f"신규진입 차단 중 — {sym}({LOOKBACK_H}h+{ret6h:.0f}%) 스킵")
                    continue
