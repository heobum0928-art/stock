"""
마진 숏 완화판 트레이더 (margin_short_wide_trader) — margin_short_trader의 자매 엔진.

★ 2026-08-25 신설. 사용자 요청: "진입만 완화하는 봇을 따로 만들고 청산은 기존처럼,
   내가 보고 팔고 싶은 건 알아서 하는 건 어떨까." — margin_short_trader.py 전체를 복사해
   PUMP_PCT/PUMP_PCT_MAX만 바꿨다. 청산(손절·트레일링·만기·서버측스탑·외부청산감지·역행경보)은
   전부 동일 코드다. 진입 범위는 기존 봇(30~40%)과 겹치지 않게 15~30%로 뒀다 — 같은 신호를
   두 엔진이 동시에 잡는 일이 구조적으로 없다(추가로 상대 엔진 포지션도 교차 제외한다, 아래 참조).

★ armed_engines에 "mshort_wide"/"mshort_wide_fut"가 없으면 이 봇은 계속 dry(모의)로만 돈다
   (MarginGuard/BinanceGuard의 _gate()가 armed_engines를 확인 — 코드로 검증 완료).
   **실거래 전환은 사용자가 직접 두 config 파일에 엔진을 추가해야 한다(CLAUDE.md 6항).**

★ 원본 규칙(2026-07-12 확정, 2026-07-22 재검증 후 파라미터만 갱신): LOOKBACK_H시간 +PUMP_PCT% 급등
...
★ margin_guard OFF면 자동 dry. 지금은 armed_engines 미등록이라 항상 dry(모의) — 안전.
포지션 data/margin_short_wide_pos.json | 기록 data/margin_short_wide_trades.csv | 로그 logs/margin_short_wide_trader.log
...
def _single():
    ...
    try: _sock.bind(("127.0.0.1", 47299))
    except OSError: print("[ERROR] margin_short_wide_trader 이미 실행 중 (포트 47299)."); sys.exit(1)
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [MSHORT_WIDE] %(message)s",
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("logs/margin_short_wide_trader.log", encoding="utf-8")])
...
# ★ 2026-08-25: 완화판 진입범위 — 기존 봇(30~40%)과 안 겹치게 15~30%로 설정.
#   2026-08-25 사전등록 PREREG_PUMP_LOWER.md에서 봇 혼자 처리 시(현행 손절·트레일링 그대로)
#   15~20%/20~30% 추가는 둘 다 총손실이 늘어 기각됐다(건당 평균은 오히려 작아짐 -1.77%/-3.29%인데
#   건수가 늘어 총액이 커짐). 이 봇은 그 결론을 뒤집으려는 게 아니라, "진입은 넓히고 사람이
#   직접 지켜보며 자른다"는 다른 조합을 실거래로 테스트하기 위한 것 — 이 조합은 사람 판단이
#   껴 있어 과거 데이터로 검증이 불가능하다(2026-08-25 사용자 대화 참고). 봇 혼자 두면 이미
#   기각된 조합을 그대로 재현할 뿐이라는 점을 사용자에게 명시적으로 전달했다.
# ★ 2026-09-01 사용자 지시("우선 신규진입만 차단해"): 완화봇 신규진입 중단.
#   근거: 1년 백테스트 평균 -2.01%(증거금 기준, wide_v1_bt.py) + 실전에서 같은 패턴 재현
#   (승률 높은데 합계 마이너스). 기존 포지션의 손절·만기·펀딩·외부청산 관리는 전부 유지된다
#   — armed_engines에서 빼면 청산까지 막히므로 이 스위치를 쓴다. 재개하려면 True로.
# ★ 2026-09-02 사용자 결정(선택지 3): CUSUM 상위10% 필터를 걸고 재개. 무필터로 돌리던
#   것과 다른 조건이다 — 필터 적용 시 -2.01% → +1.53%로 뒤집힌다(아래 CUSUM_THRESHOLD 주석).
...
HOLD_H = 48
STOP_PCT = 5.0            # ★ 2026-09-22 사용자 결정("10%로 하자", 증거금 -10%): PREREG_STOP10.md —
                          # 평균은 판별불가지만 손익비 0.39→2.04로 개선(이길 때 커지고 질 때 작아짐).
                          # 이전값 40.0(증거금 -80%, 원본과 동일했던 값). 진입가 대비 +5% 상승 시 손절
...
BUF_PATH = ROOT / "data" / "margin_short_wide_buf.json"
POS_PATH = ROOT / "data" / "margin_short_wide_pos.json"
OTHER_ENGINE_POS_PATH = ROOT / "data" / "margin_short_pos.json"  # 원본 봇 — 읽기 전용, 겹침 방지용
TRADES_PATH = ROOT / "data" / "margin_short_wide_trades.csv"
...
LISTING_AGE_CACHE_PATH = ROOT / "data" / "_listing_age_cache_wide.json"
NEWLISTING_POS_PATH = ROOT / "data" / "newlisting_long_paper_wide_pos.json"
...
# ★★ 2026-09-02 같은 날 철회 — 위 근거가 틀렸다. 강세장 데이터(pq_bull) 확보 후 재검증하니
#   결론이 뒤집혔다. 위에 인용한 "제외되는 강세 신호 n=75, -3.24%"는 신뢰구간이
#   [-15.17, +5.02]로 사실상 정보가 없는 표본이었는데 그걸 근거로 삼았다.
#   강세장 데이터 재검증 (CUSUM 통과분을 BTC 30일 수익률로 분할):
#     BTC강세    n=223  평균 +5.46%  95%CI [+1.72,+8.98]  ← 0 배제, 오히려 더 좋다
#     비강세     n=350  평균 +1.42%
#   ★ 상호작용이 핵심: CUSUM **없이** 보면 강세장이 나쁘고(-2.27% vs +0.35%),
#     CUSUM **적용 후**엔 강세장이 더 좋다(+5.46% vs +1.42%). CUSUM이 "진짜 이상급등"만
#     남기므로, 강세장의 과열된 급등일수록 되돌림이 세다는 해석과 일치한다.
#     즉 이 필터는 CUSUM과 결합하면 좋은 신호를 버린다.
#   교훈: 필터를 단독 검정하지 말고 **실제 배포 조합(CUSUM 적용 후)에서** 검정할 것,
#   그리고 소표본(n=75, CI 폭 20%p)을 근거로 배포하지 말 것.
BTC_BULL_SKIP_ON = False
...
def _klines_any(sym: str, interval: str, limit: int):
    ...
    조용히 실패했다(margin_short_trader.py와 동일 수정). 반환: 캔들 리스트 또는 None."""
...
def _listing_age_days(coin: str, cache: dict) -> float | None:
    ...
    캐시 TTL 7일. ★ 2026-09-01: 선물 전용 코인 폴백 포함(_klines_any)."""
    now = time.time()
    c = cache.get(coin)
...
BORROWABLE_PATH = ROOT / "data" / "_borrowable_all_wide.txt"
...
def refresh_borrowable():
    ...
    try:
        ...
        try: notify.send(f"[완화] ⚠️ 마진숏봇: 대출가능목록 갱신 실패 — {e} (IP차단·API권한 문제 의심)")
...
FUTURES_ENGINE = "mshort_wide_fut"
FUTURES_UNIVERSE_PATH = ROOT / "data" / "_futures_tradeable_wide.txt"
...
def all_tickers():
    ...
    ★ 2026-09-01: 현물에 없는 심볼(선물 전용 347개)은 선물 티커로 보충 —
    margin_short_trader.py와 동일 수정(상세는 그쪽 all_tickers 주석)."""
...
# ★ 2026-08-31: 사용자 지시로 취소 — 다시 켜려면 True로.
# ★ 2026-09-02: 재활성화(사용자 선택지 3). 문턱을 6.59 → 38.1로 올렸다.
#   6.59는 "하루 4~5건"이라는 빈도 목표에서 역산한 값이라 근거가 얇았다(평균 -0.36%,
#   95%CI 0 포함). 38.1은 **원본봇과 동일한 규칙(상위 10% 컷)**을 완화봇 신호셋에
#   그대로 적용해 나온 값 — 결과를 보고 고른 게 아니라 같은 원칙을 옮긴 것이다.
#   검증(cusum_wide_result.pkl, n=881/년 = 하루 2.4건):
#     평균 +1.53%(필터 전 -2.01%), 승률 69.5%, 훈련 +1.18% / 홀드아웃 +2.71% 부호일치,
#     최대기여 1건 제외 +1.41%, 최악10 -93.4%(필터 전 -100.0%)
#   ★ 단 95%CI [-1.22,+4.30]로 0을 포함한다 — 원본봇 필터(99%CI까지 양수)와 달리
#     "확실히 플러스"가 아니라 "확실한 마이너스에서 벗어남" 수준의 근거임을 명시해둔다.
# ★ 2026-09-17 사용자 결정으로 OFF. 원장상 적용 전 48건 +0.71% vs 후 57건 +0.64%(증거금)로 차이 없음.
#   단 2026-08-31 백테스트는 무필터 -2.01% / 필터 +1.53%였다(09-02 재개 조건).
# ★ 2026-09-18 사용자 결정으로 다시 ON — 완화봇 재개 조건을 09-02와 동일하게 맞춘다.
CUSUM_ENABLED = True
CUSUM_VOLWIN = 288
CUSUM_K = 0.3
CUSUM_THRESHOLD = 38.1      # 완화봇 신호셋 상위10%컷(원본봇과 동일 규칙)
...
def log_trade(row):
    ...
    try:
        ...
        try:
            notify.send(f"[완화] 🚨 거래기록 실패 {row.get('symbol')} — CSV가 열려있는지 확인 필요")
        except Exception:
            pass
...
def main():
    ...
    try: notify.send(f"[완화] 📉 마진숏 트레이더 시작 [{mode}] — {LOOKBACK_H}h+{PUMP_PCT:.0f}% 급등주 숏, 대출가능 {len(UNIVERSE)}코인+선물폴백")
    except Exception: pass
        ...
        try:
            ...
            for sym in list(positions.keys()):
                ...
                if pos.get("live") and now - pos.get("_ext_chk", 0) >= EXT_CHECK_SEC:
                    ...
                    if exchange_position_gone(sym, pos["coin"], pos.get("venue", "margin"),
                        ...
                        # ★ 2026-09-01(버그감사 3위 수정): 무조건 카운터 리셋이 아니라, 청산가가
                        #   손절선 부근(-STOP_PCT+2%p 이내)이면 서버측 스탑 체결로 간주해 스트라이크를
                        #   올린다 — 기존엔 서버스탑 체결이 "외부청산"으로 분류되며 연속손절 카운터가
                        #   지워져 블랙리스트가 무력화됐다(BMT -40.08% 실사례). 진짜 수동청산은 리셋 유지.
                        ...
                        if pnl_pct <= -(STOP_PCT - 2):
                            ...
                            if _st[_ck] >= 2:
                                ...
                                try: notify.send(f"[완화] ⛔ {_ck} 연속 {_st[_ck]}회 손절 — 7일간 신규진입 제외")
                                except Exception: pass
                        else:
                            _st[_ck] = 0
                ...
                if (SERVER_STOP_MARGIN
                    ...
                    if sres.get("deferred"):
                        ...
                        try: notify.send(f"[완화] 🛡 {sym} 서버측 손절 등록 — 봇 다운 시에도 +{STOP_PCT:.0f}%에서 자동청산")
                        except Exception: pass
                    else:
                        ...
                        if fails == 1:
                            try: notify.send(f"[완화] ⚠️ {sym} 서버측 손절 등록 실패 — 봇 폴링 손절만 작동 중")
                            except Exception: pass
                        if fails >= 5:
                ...
                if pos.get("live") and dd >= min(ALERT_DD_LEVELS):
                    ...
                    for lv in ALERT_DD_LEVELS:
                        if dd >= lv and lv not in done:
                            ...
                            try:
                                notify.send("\n".join([
                                    f"[완화] ⚠️ {sym} 증거금 -{lv:.0f}% 도달 (현재 {-dd:+.1f}%)",
                ...
                if pos["live"] and not cres.get("live"):
                    ...
                    if fails in (1, 3) or fails % 10 == 0:
                        try: notify.send(f"[완화] 🚨 {venue_tag}숏 청산 실패 {sym} {reason} → {cres} (연속{fails}회) — 실거래소엔 포지션 열려있음! 확인 필요")
                        except Exception: pass
                    continue
                ...
                try: notify.send(f"[완화] 📈 {venue_tag}숏 청산 {sym} {reason} pnl={pnl_pct:+.1f}% ({pnl_usdt:+.1f}USDT)")
                except Exception: pass
            ...
            if bal is None:
                ...
                if api_fail in (2, 6) or api_fail % 12 == 0:
                    ...
                    try: notify.send(f"[완화] 🚨 마진숏봇: 마진잔고 조회 {api_fail}회 연속 실패 — API 문제 의심, 확인 필요")
                    except Exception: pass
            else:
                if api_fail >= 2:
                    try: notify.send(f"[완화] ✅ 마진숏봇: API 정상화 (잔고 {bal:.1f} USDT)")
                    except Exception: pass
                api_fail = 0
                if bal < 0 and not neg_balance_notified:
                    log.info(f"마진잔고 {bal:.1f}(마이너스) — 타 엔진 차입 추정, 정상. 신규진입은 고정금액 사용")
                    try: notify.send(f"[완화] ℹ️ 마진숏봇: 계좌 USDT 순자산 마이너스({bal:.1f}) — 타 엔진(재량롱 등) 정상 차입으로 추정, 신규진입은 고정금액 사용해 영향 없음")
                    except Exception: pass
            ...
            if ENGINE not in engine_caps:
                ...
                if now - last_capcfg_alert >= 1800:   # 30분에 한 번만 알림(스팸 방지)
                    try: notify.send(f"[완화] 🚨 마진숏봇: engine_caps_usdt에 {ENGINE} 설정 없음 — 신규진입 전면 차단 중, margin_live_config.json 확인 필요")
                    except Exception: pass
            ...
            for coin in (UNIVERSE_SET | FUTURES_UNIVERSE):
                ...
                # ★ 2026-09-01: 신규진입 차단 스위치 — 신규상장 모의롱 기록(위)은 유지하고
                ...
                if use_margin:
                    ...
                    if res.get("live"):
                        ...
                        try: notify.send(f"[완화] 📉 마진숏 진입 {sym} {LOOKBACK_H}h+{ret2h:.0f}% (증거금{margin:.0f}USDT)")
                        except Exception: pass
                    else:
                    ...
                    if res.get("live"):
                        ...
                        if not res.get("stop_verified"):
                            ...
                            try: notify.send(f"[완화] 🚨 {sym} 서버측 스탑 등록 실패 — 무보호 포지션, 수동 확인 필요")
                            except Exception: pass
                        log.warning(f"★실전 선물숏 진입(마진대출폴백) {sym} {LOOKBACK_H}h+{ret2h:.0f}% 증거금{margin:.0f} → {res['qty']}개 (서버스탑={res.get('stop_order_id')})")
                        try: notify.send(f"[완화] 📉 선물숏 진입(마진폴백) {sym} {LOOKBACK_H}h+{ret2h:.0f}% (증거금{margin:.0f}USDT, 서버스탑{'OK' if res.get('stop_verified') else '실패!'})")
                        except Exception: pass
                    else:
