"""
⏸️ 중단됨 (2026-08-20) — RSI 극단 과매수 숏 트레이더 (rsi_extreme_short).

★★ 2026-08-20 라벨 정정 + 운용중단:
   ① **파일명의 "_paper"가 거짓이었다.** 이름에 paper(모의)가 붙어 있으나 실제로는
      armed_engines에 들어 있어 2026-07-13부터 소액 실거래로 돌았다(주문기록 70행).
      2026-08-19 울트라감사에서 발견돼 disarm 완료. 이름은 import 경로·watchdog 등록·
      데이터 파일명이 전부 묶여 있어 바꾸지 않는다(바꾸면 그게 더 큰 사고).
   ② **2026-08-20 운용 중단** — 58건 실측 건당 -0.02%(t≈-0.04)로 무작위 대조군
      (random_trades.csv 46건 -0.15%)과 통계적으로 구분되지 않음. 아래 검증문의
      "TEST +3.70%(t_day 4.12)"는 실거래에서 재현되지 않았다.
   ③ **손절 규칙이 아예 없다**(시간만기 청산뿐) — 08-19 감사 지적사항.
   watchdog BOTS에서 제거됨. 표본 58건은 data/rsi_short_trades.csv에 보존.

[이하 원문 — 2026-07-13 실전전환 당시 기준, 이력 보존용]
